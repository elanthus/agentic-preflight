...
For lint/test, pick from `data.candidates` and re-invoke with `--command`; offer to
write it into `[commands]` so it is settled. For review, configure `[review] command`
and retry `review run` — reviewer commands are never detected. If lint/test
