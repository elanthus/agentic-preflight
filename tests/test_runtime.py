...
def test_node_project_without_a_pin_is_reported(tmp_path):
    (tmp_path / "package.json").write_text(json.dumps({"engines": {"node": ">=24 <25"}}))
...
def test_nvm_pin_is_activated_for_a_non_interactive_stage(tmp_path, monkeypatch):
    ...
    (nvm_dir / "nvm.sh").write_text(f'nvm() {{ export PATH="{fake_bin}:$PATH"; }}\n')
...
def test_missing_pinned_manager_fails_instead_of_using_system_node(tmp_path, monkeypatch):
    (tmp_path / "package.json").write_text("{}")
    (tmp_path / ".nvmrc").write_text("24\n")
...
def test_non_strict_runtime_falls_back_when_an_explicit_manager_is_missing(tmp_path, monkeypatch):
    (tmp_path / "package.json").write_text("{}")
    monkeypatch.setattr(runtime, "_binary", lambda _name: None)

    prepared = runtime.prepare_command(tmp_path, "node --version", manager="volta", strict=False)
...
def test_probe_node_reports_command_failure_and_missing_version(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        runtime.subprocess,
        "run",
        lambda *args, **kwargs: runtime.subprocess.CompletedProcess(args[0], 0, "not json\n", ""),
    )
