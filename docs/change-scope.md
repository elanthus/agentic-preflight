...
exception stays visible in `status` and the ledger rather than looking like a pass.
...
A skipped stage and a passed stage are different facts, and a ledger that renders them
identically cannot be audited. `status` and the ledger both carry the `skipped` status and
its reason, so a reader can tell which checks actually executed against a given SHA.
